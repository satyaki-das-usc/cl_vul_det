...
FUNCTION_RE = re.compile(r'\b([_A-Za-z]\w*)\b(?=\s*\()')
...
class SliceTokenizer:
    ...
    def tokenize_slice(self):
        ...
        for start, end, edge_data in self.slice_graph.edges(data=True):
            ...
            edge_string_rev = f"-{self.edge_type_map[edge_data['label']]}-"
    ...
    def clean_gadget(self, gadget: List[str]):
        ...
        for line in gadget:
            ...
            nostrlit_line = STRING_LITERAL_RE.sub('""', line)
            # remove all character literals
            nocharlit_line = CHAR_LITERAL_RE.sub("''", nostrlit_line)
