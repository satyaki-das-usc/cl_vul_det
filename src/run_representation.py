...
from src.models.vul_det import CLVulRep
...
def init_log():
    ...
    logging.basicConfig(
        handlers=[
            logging.FileHandler(join(LOG_DIR, "run_representation.log")),
...
def train(model: LightningModule, data_module: LightningDataModule,
    ...
    # tensorboard logger
    tensorlogger = TensorBoardLogger(join("ts_logger", "representation", model_name, gnn_name, sampler_name, nn_text),
                                     dataset_name)
    ...
    checkpoint_dir = join(config.model_save_dir, "representation", gnn_name, sampler_name, nn_text)
...
if __name__ == "__main__":
    ...
    if sampler == "vf":
        ...
        num_clusters = ceil(len(train_slices) / config.hyper_parameters.batch_size)
        backbone = GraphSwAVModel(config, vocab, vocab_size, pad_idx, num_clusters=num_clusters)
    ...
    model = CLVulRep(config, vocab, vocab_size, pad_idx, backbone=backbone)
