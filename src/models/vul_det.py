...
class CLVulDet(LightningModule):
    ...
    def forward(self, batch: Batch) -> torch.Tensor:
        ...
        self.graph_embeddings = graph_hid
        hiddens = self.__hidden_layers(graph_hid)
        self.embeddings = hiddens
    ...
    def get_contrastive_loss(self, labels: torch.Tensor) -> torch.Tensor:
        cos_sim = cosine_similarity(self.embeddings.unsqueeze(1), self.embeddings.unsqueeze(0))
        ...
        same_label_loss = cos_sim[same_label_mask].mean()
        contrastive_loss += 0 if torch.isnan(same_label_loss) else (1 - same_label_loss)
        diff_label_loss = cos_sim[diff_label_mask].mean()
        contrastive_loss += 0 if torch.isnan(diff_label_loss) else diff_label_loss
    ...
    def training_step(self, batch: SliceGraphBatch,
        ...
        loss = F.cross_entropy(logits, batch.labels) + self.get_contrastive_loss(batch.labels)
